...
void BookInfoManagement::BookInfoManage()
{
	...
	{
		if (Total_book != 0) {
			op_num = Face_book.OperateFace();		//书籍为不0时，加载主界面
		}
		else {
			op_num = Face_book.OperateFace2();						//书籍为0时需要添加，加载子界面
		}
		if (op_num == 6) {
			break;
		}
...
int BookInfoManagement::SwitchFunction(int op_num)
{
	...
	else if (op_num == 2)							//删除
	{
		...
		for (i = no; i < Total_book; i++) {//后面的元素覆盖前面的元素
			book[i].SetIndex(i);
			book[i - 1] = book[i]; 
		}	 
	...
	else if (op_num == 4)							//查询
	{
		...
		case 1:																		//按书号搜索
		{
			...
			for (i = 0; i < Total_book; i++)
			{
				...
				if (Begin != string::npos)//string::npos == 字符串末尾
				{
