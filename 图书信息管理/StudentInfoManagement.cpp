...
void StudentInfoManagement::StuInfoManage()
{
	...
	{
		if (Total_stu != 0) {
			op_num = Face_student.OperateFace();			//不0时，加载主界面

		}
		else {
			op_num = Face_student.OperateFace2();							//为0时需要添加，加载子界面
		}
		if (op_num == 6) {
			break;
		}
	...
	if (op_char2 == 'Y' || op_char2 == 'y') { //保存到文件
		OutStuFile();
		cout << "已保存！" << endl;
	}	
	system("pause");
